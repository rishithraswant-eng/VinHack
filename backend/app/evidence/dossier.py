...
class DossierGenerator:
    ...
    def generate_dossier(self, case_ref: str, fir_num: str, io_desig: str, evidence: SealedEvidence, case_data: dict | None = None, hop_details: list[dict] | None = None) -> str:
        ...
        io_name = io_desig
        io_badge = ""
        m = re.match(r"(.*?)\s*\(Badge\s*#?(.*?)\)", io_desig)
        if m:
        ...
        elements.append(Paragraph(f"<b>FIR/DD Number:</b> {fir_num}", self.styles['Normal']))
        elements.append(Paragraph(f"<b>Statutory Authority:</b> {case_data.get('authority', 'Sec 94 BNSS')}", self.styles['Normal']))
        elements.append(Paragraph(f"<b>Police Station:</b> {case_data.get('police_station', '')}", self.styles['Normal']))
        elements.append(Paragraph(f"<b>Jurisdiction Bench:</b> {case_data.get('jurisdiction_bench', '')}", self.styles['Normal']))
