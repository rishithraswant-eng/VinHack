...
def test_synthetic_verify_dossier_script(tmp_path):
    ...
    res = subprocess.run([sys.executable, script_path, str(ev_file)], capture_output=True, text=True, check=False)
    ...
    res2 = subprocess.run([sys.executable, script_path, str(ev_file)], capture_output=True, text=True, check=False)
