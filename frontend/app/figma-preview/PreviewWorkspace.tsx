...
export default function CaseWorkspace({ caseId, seedAddress, previewState = 'IDLE' }: { caseId: string; seedAddress: string; previewState?: string }) {
  const [traceState, setTraceState] = useState(previewState);
  const [traceResult, setTraceResult] = useState<any>({confidence:0.806,vasp_node:'vasp_exchange_dynamic',authority:'Sec 94 BNSS',seed_address:seedAddress});
  const [caseData, setCaseData] = useState<any>({confidence:0.806,vasp_node:'vasp_exchange_dynamic',authority:'Sec 94 BNSS',seed_address:seedAddress});
  ...
  useEffect(() => {
    ...
    // Preview only: no case API request.
  }, [caseId]);
